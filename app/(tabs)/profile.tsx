...
const getAvatar = (avatarId: number) => {
  ...
  return (
    <ImageBackground source={require('@/assets/images/profilebackground.png')} style={styles.backgroundImage}>
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.headerContainer}>
          <Text style={[styles.subHeader, globalFont.Nunito]}>Profile</Text>
          <TouchableOpacity style={styles.logoutContainer} onPress={() => router.navigate('/starting')}>
      ...
      <View key={index} style={[styles.moodEntry, { backgroundColor: getBackgroundColor(mood.moodIcon) }]}>       
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
          {thoughts.map((thought, index) => (
            <View key={index} style={styles.postItContainer}>
              ...
              <TouchableOpacity style={styles.postIt} onPress={() => openModal(thought)}>
                <Text style={styles.postItText}>{thought.thought.slice(0, 62)}{thought.thought.length > 50 ? '...' : ''}</Text>
